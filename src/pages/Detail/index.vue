      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{'active':spuSaleAttrValue.isChecked == '1'}" 
                  ...
                  @click="chooseIsChecked(spuSaleAttrValue,spuSaleAttr)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="buyNumber" @change="changeBuyNumber">
...
<script>
  ...
  export default {
    ...
    mounted(){
	    this.$store.dispatch('getDetail',this.$route.query.skuId)
      this.$store.dispatch('getPhoneConfig')
    },
    computed:{
      ...mapState({goodsInfo:state=>state.detail.goodsInfo}),
      ...mapGetters(['categoryView','skuInfo','spuSaleAttrList']),
      //防止skuImageList为空对象报错
      skuImageList(){
    ...
    methods:{
      //手机配置高亮设置
      chooseIsChecked(spuSaleAttrValue,spuSaleAttr){  
        //将点击的整个数组变成不选中
        spuSaleAttr.spuSaleAttrValueList.forEach(item=>item.isChecked ="0")
        //将点击的元素变成选中状态
        spuSaleAttrValue.isChecked = "1"
      },
      //商品数量
      changeBuyNumber(){
        let checkup = this.buyNumber*1 
        if(isNaN(checkup) || checkup<1){
            this.buyNumber = 1
        } else{
            this.buyNumber = parseInt(checkup)
        }
      },
      //添加到购物车
      async addToCart(){
