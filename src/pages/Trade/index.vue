  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in adressInfo" :key="item.id">
        ...
        <p  @click="defaultClick(item,adressInfo)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{goodsAddress.fullAddress}}</span>
        收货人：<span>{{goodsAddress.consignee}}</span>
        电话号码:<span>{{goodsAddress.phoneNum}}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <!-- <router-link class="subBtn" to="/pay">提交订单</router-link> -->
      <a class="subBtn" @click="subClick">提交订单</a>
    </div>
  </div>
</template>
...
import { mapGetters,mapState } from 'vuex'
  export default {
    ...
    methods:{
      //点击地址显示“默认”
      defaultClick(item,adressInfo){
        adressInfo.forEach(item =>item.isDefault = 0);
        item.isDefault = 1
      },
      //提交订单信息
      async subClick(){
      let data = {
        consignee: this.goodsAddress.consignee,
        "consigneeTel": this.goodsAddress.phoneNum,
        "deliveryAddress":this.goodsAddress.userAddress,
        "paymentWay": "ONLINE",
    ...
    computed:{
      ...mapState({adressInfo:state=>state.trade.adress,orderInfo:state=>state.trade.orderInfo}),
      //收货地址
      goodsAddress(){
        let aaa = this.adressInfo.find(item=>item.isDefault == 1) || {}
        return aaa
      }
    }
  }
